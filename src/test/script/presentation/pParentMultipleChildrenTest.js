/*
 * Copyright 2017, 2023, 2025 Olov McKie
...
QUnit.module("metadata/pParentMultipleChildrenTest.js", hooks => {
	let test = QUnit.test;
	let fixture;
	...
	const createAndInitPMultipleChildren = function() {
		pParentMultipleChildren = CORA.pParentMultipleChildren(dependencies, spec, child);
	...
	test("testFactoredViewCorrectly_ForPresentationStyle", function(assert) {
		...
		CORA.pParentMultipleChildren(dependencies, spec, child);
	...
	//	
	//	CORA.pParentMultipleChildren(dependencies, spec, createChildSpy());
	//
	...
	test("testGuiElementLink", function(assert) {
		...
		assert.strictEqual(firstAddedChild.text, "text for: someTextToPresentAsLinkText");
	...
	test("testFirstPChildRefHandlerSpecNoTitleText", function(assert) {
		setSpecCPresentation("pgGroupIdOneTextChildMinimizedDefaultNoStyleInfo");
		...
		assert.strictEqual(factoredSpec.titleText, undefined);
	});

	test("testFirstPChildRefHandlerSpecWithTitleText", function(assert) {
		setSpecCPresentation("pgGroupIdTwoTextChild");
		...
		assert.strictEqual(factoredSpec.titleText, "someTextIdForTitleText");
	});
	...
	test("testInitOneChild", function(assert) {
		...
		let pParentMultipleChildren = createAndInitPMultipleChildren();
		pParentMultipleChildren.getView();
		assertNoOfChildrenAddedToView(assert, 1);
		let viewSpy = pMultipleChildrenViewFactory.getFactored(0);
