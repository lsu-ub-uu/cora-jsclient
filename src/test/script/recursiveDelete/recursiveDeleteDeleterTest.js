...
QUnit.module("recursiveDelete/recursiveDeleteDeleterTest.js", hooks => {
	...
	test("testDeleteElementCallBackSetDeletingAndCallsDeleteForAllChildren", function(assert) {
		...
		assert.strictEqual(ajaxCallFactorySpy.getFactoredNoOfAjaxCalls(), 4);
	...
	test("deleteElementsCollections", function(assert) {
		...
		assert.strictEqual(ajaxCallFactorySpy.getFactoredNoOfAjaxCalls(), 5);
	...
	test("testPresentationsFromIncomminLinksMustBeDeletedFirst", function(assert) {
		...
		assert.strictEqual(ajaxCallFactorySpy.getFactoredNoOfAjaxCalls(), 3);
	...
	test("testPresentationsFromIncomminLinksMustBeDeletedFirst_ButFails", function(assert) {
		let viewModel = {
			elementId: 1,
			id: "someGroup",
			recordType: "metadata",
		...
		sendFailedAnswerToAjaxPresentationCall(1);

		assertElementIdSetDeleteFailed(assert, 0, 3, "404 : someError");
		assert.strictEqual(ajaxCallFactorySpy.getFactoredNoOfAjaxCalls(), 2);
		//
		sendOkAnswerToAjaxPresentationCall(0);

		assertElementIdSetToDeleted(assert, 0, 2);
	...
	const sendFailedAnswerToAjaxPresentationCall = function(callNo) {
		let callSpec = getCallSpecFromAjaxCall(callNo);
		let answer = {
			status: 404,
			response: "someError",
	...
	const assertElementIdSetDeleteFailed = function(assert, callNo, elementId, error) {
		let elementIdAndError = {
			elementId: elementId,
			errorMessage: error
		};
		assert.deepEqual(recursiveDeleteView.getDeleteFailedElement(callNo), elementIdAndError);
	};
