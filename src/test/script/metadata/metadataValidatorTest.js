...
QUnit.test("testChildRemovedWhenWriteConstraintsWithoutPermissions", function(assert) {
    this.spec.metadataId = "groupIdOneTextChildWithReadWriteConstraints";
    this.metadataChildValidatorFactory.setSpySpec(this.spySpec);
    this.recordPartPermissionCalculator.addIdToReturnFalseForWrite("metadataTextVariable_textVariableId");
    ...
    let expectedResult = createRemoveMessageWithPath(["textVariableId"]);
    assert.deepEqual(messages[0], expectedResult);
});

QUnit.test("testChildRemovedWhenWriteConstraintsWithoutPermissionsRemoveFailsIfNoDataInHolder", function(assert) {
    this.spec.metadataId = "groupIdOneTextChildWithReadWriteConstraints";
    this.metadataChildValidatorFactory.setSpySpec(this.spySpec);
    this.recordPartPermissionCalculator.addIdToReturnFalseForWrite("metadataTextVariable_textVariableId");
	this.pubSub.setThrowError();
    let metadataValidator = CORA.metadataValidator(this.dependencies, this.spec);
